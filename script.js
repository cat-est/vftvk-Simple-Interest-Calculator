function compute() {
    //Calculation and get values from the user 
    var principal = parseFloat(document.getElementById("principal").value);
    var rate = parseFloat(document.getElementById("rate").value);
    var years = parseInt(document.getElementById("years").value);
    var interest = principal * years * rate / 100;
    var yearInTheFuture = new Date().getFullYear() + years;
    //Creating the inner html text to be shown to the user
...
function validateAmount() {
    ...
    var biggerThanZero = parseInt(principal) > 0;//check for positive values
    if (!biggerThanZero) {
